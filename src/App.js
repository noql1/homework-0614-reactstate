 ...
 const itemIndex = items.findIndex((item) => item.key === key);
  ...
  const filteredItems =
    (!filterType || filterType === "all"
      ...
      // const handleItemSearch = (searchTerm, toDoItems ) => {
      //   if (!searchTerm){
      //     return (toDoItems)
      //   }
    ...
    const searchArray = items.filter(item =>
        item.label.substring(0, searchTerm.length).toLowerCase() === searchTerm.toLowerCase())

    // filteredItems = !searchTerm ? filteredArray : searchArray
          ...
          filteredItems.map((item) => (
            <li key={item.key} className="list-group-item">
              <span className={`todo-list-item${item.done ? " done" : ""}${item.important ? " important" : ""}`}>
                ...
                <button onClick={() => handleItemImportant(item)}
